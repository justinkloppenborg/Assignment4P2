...
//const API_URL = 'https://justinkloppenborg.pythonanywhere.com/';
const API_URL = 'http://justinkloppenborg.pythonanywhere.com/';
...
updateStock(stock){
  const url = `${API_URL}/api/stocks/${investment.pk}`;
  let jwtToken = localStorage.getItem('token');
  const headers = {Authorization: `jwt ${jwtToken}`};
  return axios.put(url, investment, {headers: headers});
}
...
deleteStock(stock_Pk){
   const url = `${API_URL}/api/stocks/${investment_Pk}`;
   let jwtToken = localStorage.getItem('token');
   const headers = {Authorization: `jwt ${jwtToken}`};
   return axios.delete(url, {headers: headers});
}
